...
print("•••   HELLEMPIRE  SESSION  GENERATOR   •••")
print("\nHello!! Welcome to HellEmpire Session Generator\n")
...
if okvai == "69":
    ...
    if library == "1":
        ...
        with TelegramClient(StringSession(), APP_ID, API_HASH) as hellbot:
            ...
            hellbot.send_message("me", f"#HELLxEMPIRE #HELLxEMPIRE_SESSION \n\n`{hellbot.session.save()}`")
    elif library == "2":
        ...
        with Client(':memory:', api_id=APP_ID, api_hash=API_HASH) as hellbot:
            print("\nYour HellBot Session Is sent in your Telegram Saved Messages.")
            hellbot.send_message("me", f"#HELLxEMPIRE_MUSIC #HELLxEMPIRE_SESSION\n\n`{hellbot.export_session_string()}`")
    else:
